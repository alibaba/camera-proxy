...
export interface TouchControlRotateAutoProps {
	/**
	 * 被控制的 CameraProxy
	 */
	camera: CameraProxy
...
export class TouchControlRotateAuto {
	 ...
	 */
	public scale = 1.0
	public mode: 'pan' | 'pitch' | undefined

	constructor(props: TouchControlRotateAutoProps) {
		...
		let initPitchDeltaY = 0
		let initPitchDeltaX = 0
		...
		this.hammer.on('pinchstart', (e) => {
			...
			initPitchDeltaY = e.deltaY
			initPitchDeltaX = e.deltaX

			deltaX = e.deltaX
			deltaY = e.deltaY

			console.log('initPitchDeltaY', initPitchDeltaY)
			console.log('initPitchDeltaX', initPitchDeltaX)

			if (!this.mode) {
				const absX = Math.abs(initPitchDeltaX)
				const absY = Math.abs(initPitchDeltaY)
				console.log('判断模式X', absX)
				console.log('判断模式Y', absY)
				if (absY > absX) {
					this.mode = 'pitch'
				} else {
					this.mode = 'pan'
		...
		this.hammer.on('pinch', (e) => {
			// if (!this.mode) {
			// 	const absX = Math.abs(e.deltaX - initPitchDeltaX)
			// 	const absY = Math.abs(e.deltaY - initPitchDeltaY)
			// 	console.log('判断模式X', absX)
			// 	console.log('判断模式Y', absY)
			// 	if (absY > absX) {
			// 		this.mode = 'pitch'
			...
			// pitch
			{
				...
				pitchDeltaY = e.deltaY
				if (this.mode === 'pitch') this.camera.setPitch(this.camera.pitch + dPitchDeltaY)
			}
...

			{
				...
				// this.camera.goRight(-dx / scale, props.horizontal)
				// this.camera.goUp(dy / scale, props.horizontal)
				if (this.mode === 'pan') this.camera.pan(-dx / scale, dy / scale, props.horizontal)
			}
		})
		this.hammer.on('rotate', (e) => {
			// 这种控制方案交互不方便
			const drotation = (e.rotation - rotation) * 0.02
